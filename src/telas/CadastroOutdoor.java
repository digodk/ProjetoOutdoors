...
import dados.Outdoor.TiposOutdoor;
...
public class CadastroOutdoor extends JDialog {
  ...
  private static Outdoor outdoorCadastrado, outdoorEmCadastro;
  ...
  private static void salvarDados(ActionEvent e) {
    ...
    if (dadosOK()) {
      ...
      outdoorCadastrado = Outdoor.cadastrar(outdoorEmCadastro);
      Auxiliares.dispararEventoFecharJanela(frame);
    }
  }

  public static Outdoor cadastrar() {
    return cadastrar(new Outdoor());
  }

  // ---Métodos de cadastro. É possível cadastrar um novo outdoor ou editar um existente
  public static Outdoor cadastrar(Outdoor out) {
    if (!telaCarregada) {
      try {
